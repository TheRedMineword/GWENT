...
function string_pos(haystack, needle) {
	return STRNG_lowercase(haystack).indexOf(
		STRNG_lowercase(needle)
	);
}
...
function getLeaderIndex(name) {
	return card_dict.findIndex(card =>
		card.row === "leader" &&
...
function getCardIndex(name, requestedCount = 1) {
	...
	const cards = card_dict.filter(card =>
		card.row !== "leader" &&
		STRNG_lowercase(card.name) === STRNG_lowercase(name)
	);
	...
	const index = card_dict.findIndex(card =>
		card.row !== "leader" &&
		STRNG_lowercase(card.name) === STRNG_lowercase(name)
	);
...
function getCardsIndex_return_more(
	faction,
	cards,
	minUnits = MIN_UNITS
) {
	...
	const MIN_SPECIAL = 2;
	const MAX_SPECIAL = Math.floor(
		TARGET_TOTAL * TARGET_SPECIAL_RATIO
	);
	...
	function addCard(name, count, sampleCard) {
		...
		usedCards[name] =
			getCurrentCount(name) + count;

		totalCards += count;

		if (isUnit(sampleCard))
			unitCount += count;

		if (isHero(sampleCard))
			heroCount += count;

		if (isSpecial(sampleCard))
			specialCount += count;

		if (isWeather(sampleCard))
			weatherCount += count;
	...
	function getAvailableCards(filterFn) {
		...
		return card_dict.filter(card => {
			...
			if (Number(card.count || 0) <= 0)
				return false;

			if (!filterFn(card))
				return false;

			if (!canAddCard(card.name))
				return false;
	...
	function tryAddRandom(pool, maxAdd = 1) {
		...
		const freeSlots =
			getMaxCount(card.name) -
			getCurrentCount(card.name);

		if (freeSlots <= 0)
			return false;

		const addCount = Math.min(
			irandom_range(1, maxAdd),
			freeSlots
		);
	...
	for (const [name, requestedCount] of cards) {
		...
		const matchingCards = card_dict.filter(card =>
			card.row !== "leader" &&
			STRNG_lowercase(card.name) ===
			STRNG_lowercase(name)
		);
		...
		const factionCards = matchingCards.filter(
			card => card.deck === faction
		);

		const neutralCards = matchingCards.filter(
			card =>
	...
	while (
		weatherCount < MIN_WEATHER &&
		totalCards < TARGET_TOTAL
	) {

		const added = tryAddRandom(
			getAvailableCards(isWeather),
			1
		);
	...
	while (totalCards < TARGET_TOTAL) {
		...
		const neutralLimit =
			Math.floor(
				TARGET_TOTAL *
				TARGET_NEUTRAL_RATIO
			);

		const currentNeutralCount =
			result.reduce(
				(sum, [name, count]) => {
...
function getCardsIndex(faction, cards) {
	...
	return cards
			.map(card => getCardIndex(card[0], card[1]))
		.filter(Boolean);
}
